...
@Composable
fun DoubleHeaderLazyColumnPaginated(
    data: List<Item>,
        ...
        LazyColumn(state = listState) {
            mainGroup.forEach { (type, groupedData) ->
                ...
                subGroup.forEach { (subType, subGroupedData) ->
                    ...
                    items(subGroupedData.size) { i->
                        //listState.firstVisibleItemIndex.toString()
                        itemContent(subGroupedData[i])

                        if (i >= subGroupedData.size - 1 && !endReached && !isLoading) {
                            paginationAction()
                        }
